...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!token) return notConfigured('The 31-Jan-2018 price (Historical Candle V3)')
  // A short window covers 31 Jan not being a trading day: R22 then uses the last trading day before it.
  const res = await upstoxGet<UpstoxCandleResponse>(`/v3/historical-candle/${encodeURIComponent(key)}/days/1/${FMV_DAY}/2018-01-25`, token)
