...
import { analyticsToken, badRequest, CACHE_QUOTES, json, listParam, notConfigured, upstoxGet, upstream } from '../_lib/upstox'
...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!token) return notConfigured('Live prices (LTP Quotes V3)')
  const res = await upstoxGet<UpstoxLtpResponse>(`/v3/market-quote/ltp?instrument_key=${encodeURIComponent(keys.join(','))}`, token)
